...
class Invoice(BaseModel):
    ...
    credit_note_reason_code = models.CharField(
        max_length=15,
        choices=CREDIT_NOTE_REASON_CHOICES,
        blank=True,
        default='',
        help_text='BTAE-03 — reason for the credit note, per UAE VAT Executive '
                  'Regulation Article 61(1). Required for credit notes (rule ibr-158-ae).',
    )
    ...
    # ── FTA Audit File (FAF) — Supplier Ledger Classification ───────────────────
    ...
    accounts_type = models.CharField(
        max_length=10, choices=ACCOUNTS_TYPE_CHOICES, blank=True, default='',
