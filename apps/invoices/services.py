...
def _send_buyer_invoice_email(invoice: 'Invoice') -> None:
    ...
    try:
        ...
        body_html = f"""
          <p style="margin:0;">Please find your tax invoice from <strong>{supplier_name}</strong> attached to this email.</p>
          <div style="margin:20px 0 0;background:#f8faff;border:1px solid #dde5f0;border-left:4px solid #1e3a5f;
                      border-radius:0 8px 8px 0;padding:16px 20px;">
            <table style="font-size:14px;width:100%;border-collapse:collapse;">
              <tr><td style="color:#64748b;padding:4px 0;width:40%;">Invoice No</td>
                  <td style="font-weight:700;color:#1e3a5f;">{invoice.invoice_number}</td></tr>
              <tr><td style="color:#64748b;padding:4px 0;">Issue Date</td>
                  <td style="font-weight:700;color:#0f172a;">{invoice.issue_date}</td></tr>
              <tr><td style="color:#64748b;padding:4px 0;">Amount Due</td>
                  <td style="font-weight:700;color:#0f172a;">{invoice.currency} {invoice.total_amount}</td></tr>
              <tr><td style="color:#64748b;padding:4px 0;">Status</td>
                  <td><span style="background:#d1fae5;color:#065f46;padding:2px 10px;border-radius:4px;
                            font-size:12px;font-weight:700;">VALIDATED</span></td></tr>
            </table>
          </div>
          <p style="margin:18px 0 0;font-size:13px;color:#94a3b8;">This invoice has been validated through the
          UAE PEPPOL network. For queries, contact
          <a href="mailto:{invoice.company.email or ''}" style="color:#2563eb;">{invoice.company.email or supplier_name}</a>.</p>"""

        from services.emails import send_branded_email
        send_branded_email(
            subject=subject,
            to=buyer_email,
            heading=f'Tax Invoice {invoice.invoice_number}',
            intro=f'Dear {invoice.customer.name},',
            body_html=body_html,
            preheader=f'Invoice {invoice.invoice_number} — {invoice.currency} {invoice.total_amount}',
